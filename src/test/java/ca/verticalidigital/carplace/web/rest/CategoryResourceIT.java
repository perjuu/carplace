...
import ca.verticalidigital.carplace.repository.CategoryRepository;
import ca.verticalidigital.carplace.service.criteria.CategoryCriteria;
...
class CategoryResourceIT {
    ...
    @Test
    @Transactional
    void getAllCategoriesByCarModelIsEqualToSomething() throws Exception {
        // Initialize the database
        categoryRepository.saveAndFlush(category);
        CarModel carModel;
        ...
        em.persist(carModel);
        em.flush();
        category.addCarModel(carModel);
        categoryRepository.saveAndFlush(category);
        Long carModelId = carModel.getId();

        // Get all the categoryList where carModel equals to carModelId
        defaultCategoryShouldBeFound("carModelId.equals=" + carModelId);

        // Get all the categoryList where carModel equals to (carModelId + 1)
        defaultCategoryShouldNotBeFound("carModelId.equals=" + (carModelId + 1));
    }
